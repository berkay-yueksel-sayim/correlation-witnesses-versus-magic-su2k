"""
N2 — Magic der topologischen Ordnung.  2026-06-23, Seed 2026.

ZIEL: Stabilizer-Renyi-Entropie M_2 des doubled-Fibonacci Levin-Wen-GRUNDZUSTANDS auf
einem konkreten EDGE-QUBIT-Honeycomb-Torus berechnen (echte Zahl), mit Toric-Code(Z2)-GS
auf GLEICHER Geometrie als falsifizierbarer SRE=0-Nullkontrolle.

GROUNDING (nicht geraten — reale Projekt-Quellen gelesen):
- Geometrie (Vertex-Inzidenz, Plaquette-Hex-Zyklen): adapted from the exact-diagonalization
  reference implementation of the doubled-Fibonacci Levin-Wen honeycomb torus
  (sim04e, GSD=4 vorab gepinnt + Maschinenpraezis. bestaetigt).
- Levin-Wen B_p: B_p=(1/D^2)(1 + phi*B_p^tau), D^2=phi+2; B_p^tau = 6-F-Produkt (LW2005 Eq.13,
  KEINE innere Summe). F^{ttt}_t=[[1/phi,1/sqrt(phi)],[1/sqrt(phi),-1/phi]] (PROJEKT.md).
  Anordnung der 6 F-Faktoren EXAKT wie sim04e shotA build_cycle_operator (verifiziert).
- M_2-Enumerator + Anker (M2(|0>)=0, M2(|+>)=0, M2(T|+>)=log2(4/3), Additivitaet, GHZ/Bell=0):
  Projekt B_magic r1-r3 (verifiziert). M_2(psi)=-log2( (1/2^E) sum_{P in 4^E} <psi|P|psi>^4 ).

METHODE: B_p (Eq.13) + Q_v (Branching) auf dem vollen 2^E Edge-Qubit-Raum; H=-sum_v Q_v -
sum_p B_p exakt-diagonalisiert (eigh im Q_v-erlaubten Unterraum), GS = gemeinsamer +1-Eigenraum,
EIN GS-Vektor -> exakter M_2 (4^E Pauli, schneller iterierter Enumerator). Toric: gleiche
Geometrie, A_v=Prod X / B_p=Prod Z Stabilizer -> Stabilizer-GS -> M_2 EXAKT 0 (Kill-Switch).

PRIOR-ART (nie "erster"): Wei&Liu arXiv:2503.04566 (doubled Fib = LRM-Phase, qualitativ via
GSD/Bravyi-Konig); Korbany-Ellison-Stephen-Piroli arXiv:2605.22424 (LRM via mutual information);
1D-TSRE-Subtraktion Nehra et al. arXiv:2512.16673. Beitrag hier: ERSTE explizit BERECHNETE
M_2-Zahl des real-space edge-qubit String-Net-GS + Toric-SRE=0-Control + wiederverwendb. Lock.
"""
import numpy as np
...
def pauli_expectations_fast(psi, E):
    """Robust + schnell: <psi|P|psi> fuer alle 4^E P (reell). Per Qubit eine Pauli-Achse (4)
       anbauen. Layout-Invariante: T hat IMMER Form (2,)*E (out-ket-Achsen, Reihenfolge der
       Qubits 0..E-1) gefolgt von den bisher angebauten Pauli-Achsen (4,)*done am ENDE.
       Fuer Qubit q kontrahiere Pmat[in] mit out-Achse q (Position q), neue out-Achse q + neue
       Pauli-Achse ans Ende. Am Schluss out-Achsen mit bra kontrahieren."""
    psi = (psi / np.linalg.norm(psi)).astype(complex)
    Pmat = np.stack(PAULI)                       # (4,2,2): [p,out,in]
    T = psi.reshape((2,) * E)                    # nur out-Achsen, keine Pauli-Achse
    for q in range(E):
        # T-Achsen: [out_0..out_{E-1}] (E Stueck) + [pauli_0..pauli_{q-1}] (q Stueck).
        # kontrahiere Pmat-'in' (axis 2) mit out-Achse q (an Position q).
        T = np.tensordot(T, Pmat, axes=([q], [2]))
        # tensordot entfernt out-Achse q, haengt Pmat-Restachsen (p[4], out'[2]) ANS ENDE.
        # Neue Form: [out_0..out_{q-1}, out_{q+1}..out_{E-1}, pauli_0..pauli_{q-1}, p_q, out'_q]
        # out'_q (letzte Achse) muss zurueck an Position q (out-Block). p_q bleibt am Pauli-Ende,
        # ABER liegt aktuell VOR out'_q -> nach Verschieben von out'_q nach q ist Reihenfolge ok.
        T = np.moveaxis(T, -1, q)     # out'_q ans out-Block-Ende-Slot q
        # jetzt letzte Achse = p_q (am Pauli-Block-Ende) -> korrekt.
    # T-Form: (2,)*E [out-Achsen 0..E-1] + (4,)*E [pauli-Achsen 0..E-1].
...
def M2_exact(psi, E, max_axes=6):
    """Exakter M_2. Fuer kleine E direkt; fuer grosse E chunked, damit der Zwischen-Tensor
       (4^inner x 2^E) ins RAM passt. Akkumuliert sum_P <P>^4 ueber Outer-Pauli-Chunks."""
    if E <= max_axes:
        ev = pauli_expectations_fast(psi, E)
        return -np.log2(np.sum(ev**4) / (2.0**E))
    # chunked: feste Paulis auf den ersten c Qubits (outer 4^c), Rest exakt enumeriert
    c = E - max_axes                      # # outer-Qubits
    s = _m2_sum_chunked(psi, E, c)
    return -np.log2(s / (2.0**E))

def _m2_sum_chunked(psi, E, c):
    """sum_{P in 4^E} <psi|P|psi>^4, gesplittet: outer = Pauli auf Qubits 0..c-1 (4^c),
       inner = exakte 4^(E-c)-Enumeration via pauli_expectations_fast auf dem reduzierten
       Operator. <P>=<psi| (P_out (x) P_in) |psi>. Wende P_out fest an -> phi_out (E-qubit
       Vektor), dann inner-Enumeration der Pauli auf Qubits c..E-1 von <psi| P_in |phi_out>."""
    psi = (psi/np.linalg.norm(psi)).astype(complex)
    total = 0.0
    rest = E - c
    Pmat = np.stack(PAULI)
    ...
    for combo_out in itertools.product(range(4), repeat=c):
        ...
        phi = phi.reshape(-1)
        # inner: alle 4^rest Erwartungswerte <psi| (I_out (x) P_in) |phi>
        # = bra(psi) . (P_in auf Qubits c..E-1) . phi   ; outer-Qubits Identitaet.
        ev = _inner_expectations(psi.conj(), phi, E, c)   # (4^rest,)
        total += np.sum(ev**4)
    return total

def _inner_expectations(bra, phi, E, c):
    """<bra| (I auf 0..c-1) (x) (P auf c..E-1) |phi> fuer alle 4^rest Paulis. rest=E-c."""
    rest = E - c
    Pmat = np.stack(PAULI)
    T = phi.reshape((2,)*E)
    # baue Pauli-Achsen nur fuer Qubits c..E-1
    for q in range(c, E):
        T = np.tensordot(T, Pmat, axes=([q], [2]))
        T = np.moveaxis(T, -1, q)
...
def expval_one(psi, combo, E):
    """Einzelner <psi|P_combo|psi> (fuer Sampling)."""
    t = psi.reshape((2,) * E)
    out = t
...
# =====================================================================
# 1. Geometrie (verifiziert importiert), generisch N x M
# =====================================================================
def build_geo(N, M):
    E = 3*N*M
    NV = 2*N*M
    # Vertex-Inzidenz exakt wie hilbert_2x2.vertex_incident_edges
    def vinc(v):
        NM = N*M
        if v < NM:
            i, j = v//M, v % M
...
def allowed_triple(a, b, c):
    return (a+b+c) != 1            # Fibonacci: verboten = genau eine tau
...
# =====================================================================
# 4. Toric-Code-GS (Z2) auf gleicher Geometrie
# =====================================================================
...
# =====================================================================
# 5. M_2 eines GS-Unterraums (ein deterministischer Vektor)
# =====================================================================
...
def main():
    ...
    print("="*72)
    print("N2 — Magic der topologischen Ordnung (doubled Fibonacci vs Toric)")
    print("="*72)

    print("\n[LOCK] M_2-Enumerator gegen Anker:")
    anc = anchor_locks()
    ...
    for (N, M) in [(2, 2)]:
        ...
        if len(geo['plaqs']) == 0:
            print("  keine wohlgeformten Plaquetten -> uebersprungen")
            continue
    ...
    with open(OUT/"ergebnis.json", "w", encoding="utf-8") as fh:
        json.dump(report, fh, indent=2)
    print(f"\nGeschrieben: {OUT/'ergebnis.json'}")
    return report
