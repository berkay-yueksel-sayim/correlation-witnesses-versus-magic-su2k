"""
B_magic Nachbar 1 — N1-HAERTUNG, 2026-06-23.
Zwei sich ergaenzende Magic-Zeugen, die den weichen Truncation-Befund ("k=4 ueber additive")
HART machen oder ehrlich als weich entlarven. Aufbauend auf der verifizierten multi-strand engine
(multi_strand.py): exakt dieselben Konventionen (q=e^{i pi/(k+2)}, R-Symbole, sigma-Generatoren,
Pfad-Basis B=sorted(paths(...))). NICHTS an der alten Engine geaendert.

(a) LEAKAGE-FREIES 2-Qubit-Encoding (exakt dim-4) — kein Renormierungs-Artefakt mehr.
(b) TRUNCATION-FREIE Mana(d=5) auf nativem Fusionsraum (n=6, t=0, dim 5).

PRIOR-ART (im Text/AUSWERTUNG referenziert, nie "erster"):
- Universelles Braiding -> Non-Clifford-Magic ist Korollar aus Freedman-Larsen-Wang (Universalitaet)
  + Gottesman-Knill. M_2 wurde fuer Braiding-Magic im D(S_3)-Quantendouble bereits explizit
  tabelliert (Byles et al., arXiv:2408.03377: M_2=log(16/13)). Unser Beitrag: M_2/RoM
  leakage-freier 2-Qubit-SU(2)_k-Braid-GATTER als Funktion von k (worked example, kein Erstheits-Claim).
- Mana auf SU(2)_k-Fusions-/Konformblock-Raeumen ist seit Fliss, JHEP 04 (2021) 090
  (arXiv:2011.01962) etabliert (ungerade prime d). Unser Beitrag: Mana/Wigner-Negativitaet als
  Funktion eines konkret angelegten Braid-Worts auf dem prim-dim (d=5) Fusionsraum.

Deterministisch, Seed 2026. Eigene Herleitung (kein fremder Code kopiert).
"""
import numpy as np
np.random.seed(2026)

# ======================================================================
# ENGINE (exakt aus multi_strand.py uebernommen — KEINE Konventions-Aenderung)
# ======================================================================
...
# Sanity: alle sigma_m unitaer (jede unitaere Wirkung Pflicht fuer M2/Mana-Gueltigkeit)
def assert_unitary(gens, tol=1e-9, tag=""):
    for i,g in enumerate(gens):
        err=np.max(np.abs(g.conj().T@g - np.eye(g.shape[0])))
        assert err<tol, f"sigma_{i+1} nicht unitaer ({tag}): {err:.1e}"

# ======================================================================
# (a) LEAKAGE-FREIES 2-QUBIT-ENCODING (exakt dim-4)
# ======================================================================
# n=8, total=0 Braid-Rep. Subraum S = {Pfade mit p[4]==0}.
# Struktur (verifiziert): die 4 Pfade faktorisieren in
#   Qubit A := p[2] in {0,2}  (Anyonen 1-4 fusionieren zu 0)
#   Qubit B := p[6] in {0,2}  (Anyonen 5-8 fusionieren zu 0)
...
# additive (lokal-block) Single-Qubit-Werte zur Baseline
...
def setup_2q(k):
    ...
    assert len(sub)==4, f"k={k}: Subraum p[4]==0 hat dim {len(sub)} != 4"
    ...
    # Projektor P_S (auf die 4 Subraum-Pfade)
...
# --- additive (lokal-block) Baseline: Woerter NUR aus {s1,s2,s3} x {s5,s6,s7} ---
# Diese erhalten S exakt (block-diagonal in A x B). Max additive 2-Qubit-M2 = 2 x max Single-Qubit-M2.
def additive_baseline(k, comp_idx, PS, gens, nwords=3000, length=24):
    # lokale Generatoren: sigma_1,2,3 (Gruppe A) und sigma_5,6,7 (Gruppe B). gens[i]=sigma_{i+1}.
    locA=[gens[0],gens[1],gens[2]]            # s1,s2,s3
    locB=[gens[4],gens[5],gens[6]]            # s5,s6,s7
    loc = locA+locB
    # Leakage-Check fuer lokale Generatoren: muss EXAKT 0 sein
    leakmax=max(leakage_norm(g,PS) for g in loc)
    # additive Baseline = bestmoeglicher 2-Qubit-M2 als Produkt zweier Single-Qubit-M2 (separabel)
    # -> wir messen den maximalen Single-Qubit-M2 ueber Block-A-Woerter und ueber Block-B, additiv.
    # Block-A wirkt auf Qubit A (p[2]), Block-B auf Qubit B (p[6]); auf comp 4x4 sind sie kron-Faktoren.
    def block_single_m2(blockgens):
        ...
        for _ in range(nwords):
            ...
            for s in STAB1:
                # Start |s>_A (x) |0>_B  -> nach Block A bleibt Qubit B = |0>
                psiA=np.kron(s,np.array([1,0],dtype=complex))
                out=U4@psiA
                # reduziere auf Qubit A (Qubit B unberuehrt) -> Single-Qubit-M2 von Qubit A
                # da separabel: M2_1q des Qubit-A-Zustands
                a=out.reshape(2,2)  # index (A,B)
                # B sollte |0> bleiben; nimm Spalte 0
                psA=a[:,0]
...
# --- leakage-freie ENTANGLER: Woerter aus ALLEN Generatoren (inkl. s4), die S exakt erhalten ---
def order_of_s4(k, gens, comp_idx, PS, maxord=64):
    # sigma_4 = gens[3]. Endliche Ordnung bei k=2,4,8. Pruefe Potenzen.
...
def op_schmidt_rank(U4, tol=1e-7):
    """Operator-Schmidt-Rang einer 4x4 auf 2 Qubits. ==1 <=> reines Produktgatter kron(A,B)
       (=> NICHT verschraenkend). >1 <=> echt verschraenkend."""
...
def leakfree_entanglers(k, gens, comp_idx, PS, sub, order,
    ...
    """Sammle leakage-freie Woerter U (||(I-PS)U PS||<1e-9). KERNFRAGE: ist ein leakage-freies
       Wort je VERSCHRAENKEND (op-Schmidt-Rang>1) oder immer ein Produktgatter kron(A,B)?
       Strategien: (i) Zufall lang + Filter ; (ii) strukturiert s4^m, Kommutatoren, Konjugate.
       Rueckgabe: best_m2 (max 2-Qubit-M2 ueber Komp.-Stabilizer-Inputs, leakage-frei),
       best_leak, n_lf, n_entangling, max_prod_err, max_single (max Single-Qubit-M2 je Block)."""
    D=gens[0].shape[0]
    LEAK_TOL=1e-9
    best_m2=0.0; best_leak=None; n_found=0; n_ent=0; best_word=None
    max_prod_err=0.0; max_single=0.0
    comp_stab=[]  # 2-Qubit-Stabilizer-Produkt-Inputs als Komp.-Vektoren
    for sa in STAB1:
        for sb in STAB1:
    ...
    def eval_U(U,tag):
        ...
        # max 2-Qubit-M2 ueber Komp.-Stabilizer-Inputs (leakage-frei: bleibt exakt in S)
...
def run_part_a():
    print("="*72)
    print("(a) LEAKAGE-FREIES 2-QUBIT-ENCODING (exakt dim-4, n=8, S={p[4]==0})")
    ...
    print("    n_ent = #leakage-freie Woerter mit op-Schmidt-Rang>1 (= echt verschraenkend)")
    print("    lf_max = max 2-Qubit-M2 ueber leakage-freie Woerter (Stabilizer-Inputs)")
    ...
    for k in [2,4,8,3,5]:
        ...
        # LOCK k=2: 2-Qubit-M2 = 0 (Clifford), Leakage exakt 0
        if k==2:
            ...
            # HART-LOCK: jedes leakage-freie Wort ist ein EXAKTES Produktgatter kron(A,B)
            assert n_ent==0, f"k={k}: {n_ent} verschraenkende leakage-freie Woerter gefunden (Befund waere HART)!"
            assert prod_err<1e-9, f"k={k}: leakage-freie Woerter nicht exakt Produkt (prodErr {prod_err:.1e})"
            # lf_max darf 2xsingle nicht ueberschreiten (Produkt-Schranke)
            assert lf_max <= two_single+1e-6, f"k={k}: lf_max {lf_max:.4f} > 2xsingle {two_single:.4f}"
    print("    BEFUND (a): n_ent==0 fuer alle k -> JEDES leakage-freie Wort ist Produktgatter kron(A,B).")
    print("    => KEINE super-additive 2-Qubit-Magic. lf_max <= 2 x Single-Qubit (= additiv). (siehe AUSWERTUNG)")
    return rows

# ======================================================================
# (b) TRUNCATION-FREIE Mana(d=5) — Gross-2006 diskrete Wigner-Funktion, prime d=5
# ======================================================================
# Nativer Fusionsraum n=6,t=0 hat dim 5 (fuer k>=3). 5 prim -> Wigner wohldefiniert.
# Heisenberg-Weyl d=5: X|j>=|j+1 mod 5>, Z|j>=w^j|j>, w=exp(2pi i/5).
# Displacement D(a,b)=w^{ -inv2 * a*b } Z^b X^a  (inv2 = (d+1)/2 = 3 fuer d=5)  [Gross 2006, prime d]
...
def build_HW(d=5):
    ...
    inv2=(d+1)//2  # multiplikatives Inverses von 2 mod d (d ungerade prim)
...
def mana(psi):
    W=wigner(psi)
    return np.log2(sum(abs(v) for v in W.values()))  # log2-Basis (Serien-Konvention, RUN-3-Nachzug2 2026-07-05; war np.log/ln — Normierung Sigma_u A(u)=d*I identisch zu r9, verifiziert)
...
def part_b_locks():
    ...
    # LOCK (Pflicht): jeder Basiszustand |j> -> Wigner >=0 -> Mana=0
    maxbase=0.0
    for j in range(d5):
        e=np.zeros(d5,dtype=complex); e[j]=1.0
        m=mana(e); maxbase=max(maxbase,abs(m))
        W=wigner(e); negmin=min(W.values())
        assert m<1e-9, f"Basiszustand |{j}> hat Mana {m:.2e} != 0 (Lock verletzt)"
        assert negmin>-1e-12, f"Basiszustand |{j}> hat negative Wigner {negmin:.2e}"
    print(f"    STABILIZER-INPUT-LOCK: max|Mana(|j>)|={maxbase:.1e} (alle 5 Basiszustaende, Wigner>=0) -> 0 OK")
    ...
    assert mg>1e-3, "generischer Zustand nicht magisch -> Mana-Implementierung verdaechtig"
    return maxbase, mg

def run_part_b():
    maxbase, mg = part_b_locks()
    print("    ANWENDUNG: max Mana ueber Braid-Orbit der dim-5-Zustaende (encoding-frei, HW-struktur-abh.)")
    print("    HINWEIS: k=2 hat dim(n=6,t=0)=4 != 5 -> d=5-Wigner NICHT nativ anwendbar (ehrlich uebersprungen).")
    print(f"    HINWEIS: k=2->Mana-0-Lock gilt hier NICHT (war d=2-Aussage). k=2-Wert wird NICHT erzwungen.")
    ...
    for k in [2,3,4,5,8]:
        ...
        best=0.0; cnt=0
        if k in (2,4,8):
            # endlich/strukturiert: enumeriere kurze Woerter via BFS bis cap
            from itertools import product
            gg=gens+[np.linalg.inv(g) for g in gens]
            seen=set(); frontier=[np.eye(D,dtype=complex)]
            # BFS ueber Gruppe (kanonisiere via gerundete Matrix)
...
if __name__=="__main__":
    print("#"*72)
    print("# N1-HAERTUNG — Seed 2026 — zwei Magic-Zeugen")
    print("#"*72)
    rows_a = run_part_a()
    ...
    print(f"Mana stabilizer-input-Lock: max|Mana(|j>)|={mana_lock:.1e} (=0) ; Zufall={mana_rand:.4f}")

    # Provenance-Deposit fuer (b) Mana(d=5), log2-Basis (RUN-3-Nachzug2 2026-07-05).
    # Teil (a) hat kein JSON (nicht Teil dieser Freigabe, unberuehrt).
    import json
    json.dump({'basis':'log2','stabilizer_input_lock_maxabs':float(mana_lock),'random_control':float(mana_rand),
               'rows_b':[{'k':k,'dim':D,'maxMana':(float(best) if best is not None else None),'orbit':cnt,'note':note}
                         for (k,D,best,cnt,note) in rows_b]},
              open('p5a_ergebnis_hardening_b.json','w'),indent=2)
    print("Geschrieben: p5a_ergebnis_hardening_b.json")
