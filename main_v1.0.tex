...
Table~\ref{tab:map} with $V_1{=}V_2$) while the single-qubit fusion channel
...
prediction. We further probe the $k=4$ dissociation at its shared $d=3$ interface with
Kochen--Specker--Klyachko contextuality, where the KCBS witness shows only a weak, nongeneric
correlation with magic. Two reconciliations with the literature are made explicit: the present result contradicts
...
Bell inequality at all. Both directions concern whether, and how, a
...
KCBS contextuality, left open for a forthcoming companion analysis,
...
by two further computations sharing verified conventions, agree
exactly (Clifford floor $0$ at $k=2$, a finite non-Clifford
plateau at $k=4,6,8$, and the dense-representation ceiling
$\log_2(3/2)=0.585$ at $k=3,5,7,9,10$), and the two $K_3$ columns
...
equal-time-step protocol ($V_1{=}V_2$), maximized over
states, braid words, and measurement axis. $K_3(Q{=}\hat z)$: same
...
$\log_2(3/2)=0.585$ (dashed line). At every other finite or dense $k$
the witness exceeds its classical bound and is correspondingly not
blind. The $k=8$ point shows the fixed-axis value $K_3(Q{=}\hat z)$;
...
Meanwhile, the single-qubit fusion channel of the
same braid generator carries near-maximal magic,
$M_2=0.5585$, i.e.\ $95\%$ of the finite-dimensional ceiling
...
Clifford property is the driver. What is sufficient for blindness is
\emph{alignment}: the fixed measurement axis coincides with a
sufficiently high-order symmetry axis of the orbit (here, $\hat z$ is
the threefold axis of the $k=4$ tetrahedral orbit), forcing the
...
Theorem 1: an integer value would indicate a stabilizer ground space
and no certificate). We reproduce this value by a second, genuinely
independent route: the vacuum column of the doubled-Fibonacci modular
$S$-matrix extracted from a $3\times3$ lattice (Wilson-loop plus
$C_3$-rotation channel, with no analytic quantum dimensions or $F/R$
symbols entering the extraction) gives normalized weights
$\lvert S_{0a}\rvert^2$ whose Shannon entropy agrees with the direct
evaluation to $\lvert\Delta\rvert=1.8\times10^{-11}$, at the level of
the lattice-extraction residual. (The closed-form rearrangement
...
binary-entropy toric-code $T$-state of Korbany's Eq.~(35),
...
Yan~\cite{Ding2025}), a robust \emph{lower bound}. Minimizing instead over
the full four-dimensional ground space (gauge-invariant construction)
gives a range $M_2\in[6.5,8.9]$, i.e.\ $\gg0$ under every
representative, the genuine long-range-magic certificate. As a
...
%% ====================================================================
\section{Hardware-oriented prediction}
\label{sec:hw}
...
Chou2026} at qutrit dimension $d=3$, evaluated on the same
fusion-channel qutrit used throughout this paper. At $k=4$ the KCBS
...
($-0.03,-0.19,-0.07,-0.01$ respectively; consistently negative and,
at its largest magnitude ($k=4$), still only $19\%$ of the way to a
perfect correlation), which we read as evidence against a
...
non-Abelian long-range-magic survey of Zhang \emph{et
...
\href{https://doi.org/10.5281/zenodo.21134455}{doi:10.5281/zenodo.21134455}
and released under the Creative Commons Attribution 4.0 International
(CC~BY~4.0) license. The deposited outputs include the gauge-invariant
...
M.~Freedman, M.~Larsen, and Z.~Wang,
...
\href{https://arxiv.org/abs/2605.22424}{arXiv:2605.22424}.
